import { ArrowDown, ArrowRight, Menu,  ShoppingCart, X } from "lucide-react";
            ...
            <div className="flex-col block sm:hidden">
                ...
                <div className={`${isOpen ? "w-80" : "w-0"} fixed top-0 left-0 flex flex-col min-h-screen text-sm text-white bg-stone-800 w-80 transition-all duration-300`}>
                    <div className={`${!isOpen && "hidden"} flex flex-col justify-between gap-4 p-5`}>
                        ...
                        <div className="inline-block group">
                            <button className="flex items-center text-center">
                                <Link to="/misviajes" onClick={() => setIsOpen(!isOpen)}>MIS VIAJES</Link>
                                <span>
                                    <svg className="w-4 h-4 transition duration-150 ease-in-out transform fill-current group-hover:-rotate-180" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                                        <path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/>
                                    </svg>
                            ...
                            <div className="absolute flex flex-col text-center transition-transform duration-150 ease-in-out origin-top transform scale-0 bg-slate-100 rounded-xl group-hover:scale-100 min-w-32">
                                <Link to="/misviajes/América" className="px-3 py-1 font-serif text-xs text-slate-400 hover:bg-gray-500 rounded-xl" onClick={() => setIsOpen(!isOpen)}>AMERICA</Link>
                                <Link to="/misviajes/Asia" className="px-3 py-1 font-serif text-xs text-slate-400 hover:bg-gray-500 rounded-xl" onClick={() => setIsOpen(!isOpen)}>ASIA</Link>
                                <Link to="/misviajes/Asia" className="px-3 py-1 font-serif text-xs text-slate-400 hover:bg-gray-500 rounded-xl" onClick={() => setIsOpen(!isOpen)}>EUROPA</Link>
                            </div>      
                        </div>
                        <div className="inline-block group">
                            <button className="flex items-center text-center">
                                <Link to="/misfotos" onClick={() => setIsOpen(!isOpen)}>MIS FOTOS</Link>
                                <span>
                                    <svg className="w-4 h-4 transition duration-150 ease-in-out transform fill-current group-hover:-rotate-180"xmlns="http://www.w3.org/2000/svg"viewBox="0 0 20 20">
                                        <path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/>
                                    </svg>
                            ...
                            <div className="absolute flex flex-col text-center transition-transform duration-150 ease-in-out origin-top transform scale-0 bg-slate-100 rounded-xl group-hover:scale-100 min-w-32">
                                <Link to="/misfotos/retratos" className="px-3 py-1 font-serif text-xs text-slate-400 hover:bg-gray-500 rounded-xl" onClick={() => setIsOpen(!isOpen)}>RETRATOS</Link>
                                <Link to="/misfotos/paisajes" className="px-3 py-1 font-serif text-xs text-slate-400 hover:bg-gray-500 rounded-xl" onClick={() => setIsOpen(!isOpen)}>PAISAJES</Link>
                                <Link to="/misfotos/lh" className="px-3 py-1 font-serif text-xs text-slate-400 hover:bg-gray-500 rounded-xl" onClick={() => setIsOpen(!isOpen)}>LUGARES HISTORICOS</Link>
                                <Link to="/misfotos/ciudades" className="px-3 py-1 font-serif text-xs text-slate-400 hover:bg-gray-500 rounded-xl">CIUDADES</Link>
                            </div>
                        </div>
